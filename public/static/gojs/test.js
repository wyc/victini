...
$packages["main"] = (function() {
	var $pkg = {}, jquery, sliceType, funcType, jQuery, main;
	...
	main = function() {
		...
		console.log("asdf");
		jQuery(new sliceType([new $String("input#name")])).On(new sliceType([new $String("keyup"), new funcType((function(e) {
			var name;
			name = jQuery(new sliceType([new $js.container.ptr(e.Object.target)])).Val();
			name = jquery.Trim(name);
			console.log("name is" + name);
			if (name.length > 0) {
				jQuery(new sliceType([new $String("span#output")])).SetText(new $String("Log in as " + name + " !"));
			} else {
				jQuery(new sliceType([new $String("span#output")])).Empty();
			}
		}))]));
	};
	$pkg.$init = function() {
